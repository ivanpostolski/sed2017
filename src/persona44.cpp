...
Persona44::Persona44(const string &name) :
Atomic(name),
name(name),
...
int Persona44::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona44::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona44::setearRelojDeInfeccion(){
...
void Persona44::setearRelojDeVacunacion(){
...
Model &Persona44::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona44::internalFunction(const InternalMessage &)
{
	...
	Model &Persona44::outputFunction(const CollectMessage &msg)
	{
