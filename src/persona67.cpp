...
Persona67::Persona67(const string &name) :
Atomic(name),
name(name),
...
int Persona67::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona67::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona67::setearRelojDeInfeccion(){
...
void Persona67::setearRelojDeVacunacion(){
...
Model &Persona67::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona67::internalFunction(const InternalMessage &)
{
	...
	Model &Persona67::outputFunction(const CollectMessage &msg)
	{
