...
Persona19::Persona19(const string &name) :
Atomic(name),
name(name),
...
int Persona19::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona19::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona19::setearRelojDeInfeccion(){
...
void Persona19::setearRelojDeVacunacion(){
...
Model &Persona19::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona19::internalFunction(const InternalMessage &)
{
	...
	Model &Persona19::outputFunction(const CollectMessage &msg)
	{
