...
Persona54::Persona54(const string &name) :
Atomic(name),
name(name),
...
int Persona54::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona54::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona54::setearRelojDeInfeccion(){
...
void Persona54::setearRelojDeVacunacion(){
...
Model &Persona54::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona54::internalFunction(const InternalMessage &)
{
	...
	Model &Persona54::outputFunction(const CollectMessage &msg)
	{
