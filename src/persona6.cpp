...
Persona6::Persona6(const string &name) :
Atomic(name),
name(name),
...
int Persona6::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona6::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona6::setearRelojDeInfeccion(){
...
void Persona6::setearRelojDeVacunacion(){
...
Model &Persona6::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona6::internalFunction(const InternalMessage &)
{
	...
	Model &Persona6::outputFunction(const CollectMessage &msg)
	{
