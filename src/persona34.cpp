...
Persona34::Persona34(const string &name) :
Atomic(name),
name(name),
...
int Persona34::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona34::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona34::setearRelojDeInfeccion(){
...
void Persona34::setearRelojDeVacunacion(){
...
Model &Persona34::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona34::internalFunction(const InternalMessage &)
{
	...
	Model &Persona34::outputFunction(const CollectMessage &msg)
	{
