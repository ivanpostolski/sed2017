...
Persona28::Persona28(const string &name) :
Atomic(name),
name(name),
...
int Persona28::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona28::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona28::setearRelojDeInfeccion(){
...
void Persona28::setearRelojDeVacunacion(){
...
Model &Persona28::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona28::internalFunction(const InternalMessage &)
{
	...
	Model &Persona28::outputFunction(const CollectMessage &msg)
	{
