...
Persona27::Persona27(const string &name) :
Atomic(name),
name(name),
...
int Persona27::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona27::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona27::setearRelojDeInfeccion(){
...
void Persona27::setearRelojDeVacunacion(){
...
Model &Persona27::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona27::internalFunction(const InternalMessage &)
{
	...
	Model &Persona27::outputFunction(const CollectMessage &msg)
	{
