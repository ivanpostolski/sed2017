...
Persona56::Persona56(const string &name) :
Atomic(name),
name(name),
...
int Persona56::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona56::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona56::setearRelojDeInfeccion(){
...
void Persona56::setearRelojDeVacunacion(){
...
Model &Persona56::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona56::internalFunction(const InternalMessage &)
{
	...
	Model &Persona56::outputFunction(const CollectMessage &msg)
	{
