...
Persona40::Persona40(const string &name) :
Atomic(name),
name(name),
...
int Persona40::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona40::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona40::setearRelojDeInfeccion(){
...
void Persona40::setearRelojDeVacunacion(){
...
Model &Persona40::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona40::internalFunction(const InternalMessage &)
{
	...
	Model &Persona40::outputFunction(const CollectMessage &msg)
	{
