...
Persona4::Persona4(const string &name) :
Atomic(name),
name(name),
...
int Persona4::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona4::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona4::setearRelojDeInfeccion(){
...
void Persona4::setearRelojDeVacunacion(){
...
Model &Persona4::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona4::internalFunction(const InternalMessage &)
{
	...
	Model &Persona4::outputFunction(const CollectMessage &msg)
	{
