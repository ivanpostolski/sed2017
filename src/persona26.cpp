...
Persona26::Persona26(const string &name) :
Atomic(name),
name(name),
...
int Persona26::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona26::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona26::setearRelojDeInfeccion(){
...
void Persona26::setearRelojDeVacunacion(){
...
Model &Persona26::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona26::internalFunction(const InternalMessage &)
{
	...
	Model &Persona26::outputFunction(const CollectMessage &msg)
	{
