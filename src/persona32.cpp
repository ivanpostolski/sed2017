...
Persona32::Persona32(const string &name) :
Atomic(name),
name(name),
...
int Persona32::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona32::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona32::setearRelojDeInfeccion(){
...
void Persona32::setearRelojDeVacunacion(){
...
Model &Persona32::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona32::internalFunction(const InternalMessage &)
{
	...
	Model &Persona32::outputFunction(const CollectMessage &msg)
	{
