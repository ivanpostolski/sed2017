...
Persona65::Persona65(const string &name) :
Atomic(name),
name(name),
...
int Persona65::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona65::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona65::setearRelojDeInfeccion(){
...
void Persona65::setearRelojDeVacunacion(){
...
Model &Persona65::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona65::internalFunction(const InternalMessage &)
{
	...
	Model &Persona65::outputFunction(const CollectMessage &msg)
	{
