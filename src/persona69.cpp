...
Persona69::Persona69(const string &name) :
Atomic(name),
name(name),
...
int Persona69::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona69::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona69::setearRelojDeInfeccion(){
...
void Persona69::setearRelojDeVacunacion(){
...
Model &Persona69::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona69::internalFunction(const InternalMessage &)
{
	...
	Model &Persona69::outputFunction(const CollectMessage &msg)
	{
