...
Persona2::Persona2(const string &name) :
Atomic(name),
name(name),
...
int Persona2::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona2::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona2::setearRelojDeInfeccion(){
...
void Persona2::setearRelojDeVacunacion(){
...
Model &Persona2::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona2::internalFunction(const InternalMessage &)
{
	...
	Model &Persona2::outputFunction(const CollectMessage &msg)
	{
