...
Persona8::Persona8(const string &name) :
Atomic(name),
name(name),
...
int Persona8::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona8::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona8::setearRelojDeInfeccion(){
...
void Persona8::setearRelojDeVacunacion(){
...
Model &Persona8::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona8::internalFunction(const InternalMessage &)
{
	...
	Model &Persona8::outputFunction(const CollectMessage &msg)
	{
