...
Persona68::Persona68(const string &name) :
Atomic(name),
name(name),
...
int Persona68::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona68::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona68::setearRelojDeInfeccion(){
...
void Persona68::setearRelojDeVacunacion(){
...
Model &Persona68::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona68::internalFunction(const InternalMessage &)
{
	...
	Model &Persona68::outputFunction(const CollectMessage &msg)
	{
