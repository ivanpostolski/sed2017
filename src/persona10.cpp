...
Persona10::Persona10(const string &name) :
Atomic(name),
name(name),
...
int Persona10::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona10::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona10::setearRelojDeInfeccion(){
...
void Persona10::setearRelojDeVacunacion(){
...
Model &Persona10::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona10::internalFunction(const InternalMessage &)
{
	...
	Model &Persona10::outputFunction(const CollectMessage &msg)
	{
