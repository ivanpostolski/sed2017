...
Persona37::Persona37(const string &name) :
Atomic(name),
name(name),
...
int Persona37::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona37::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona37::setearRelojDeInfeccion(){
...
void Persona37::setearRelojDeVacunacion(){
...
Model &Persona37::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona37::internalFunction(const InternalMessage &)
{
	...
	Model &Persona37::outputFunction(const CollectMessage &msg)
	{
