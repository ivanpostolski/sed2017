...
Persona38::Persona38(const string &name) :
Atomic(name),
name(name),
...
int Persona38::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona38::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona38::setearRelojDeInfeccion(){
...
void Persona38::setearRelojDeVacunacion(){
...
Model &Persona38::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona38::internalFunction(const InternalMessage &)
{
	...
	Model &Persona38::outputFunction(const CollectMessage &msg)
	{
