...
Persona41::Persona41(const string &name) :
Atomic(name),
name(name),
...
int Persona41::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona41::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona41::setearRelojDeInfeccion(){
...
void Persona41::setearRelojDeVacunacion(){
...
Model &Persona41::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona41::internalFunction(const InternalMessage &)
{
	...
	Model &Persona41::outputFunction(const CollectMessage &msg)
	{
