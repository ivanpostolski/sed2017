...
Persona52::Persona52(const string &name) :
Atomic(name),
name(name),
...
int Persona52::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona52::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona52::setearRelojDeInfeccion(){
...
void Persona52::setearRelojDeVacunacion(){
...
Model &Persona52::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona52::internalFunction(const InternalMessage &)
{
	...
	Model &Persona52::outputFunction(const CollectMessage &msg)
	{
