...
Persona16::Persona16(const string &name) :
Atomic(name),
name(name),
...
int Persona16::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona16::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona16::setearRelojDeInfeccion(){
...
void Persona16::setearRelojDeVacunacion(){
...
Model &Persona16::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona16::internalFunction(const InternalMessage &)
{
	...
	Model &Persona16::outputFunction(const CollectMessage &msg)
	{
