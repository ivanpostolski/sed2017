...
Persona12::Persona12(const string &name) :
Atomic(name),
name(name),
...
int Persona12::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona12::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona12::setearRelojDeInfeccion(){
...
void Persona12::setearRelojDeVacunacion(){
...
Model &Persona12::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona12::internalFunction(const InternalMessage &)
{
	...
	Model &Persona12::outputFunction(const CollectMessage &msg)
	{
