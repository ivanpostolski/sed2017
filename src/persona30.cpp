...
Persona30::Persona30(const string &name) :
Atomic(name),
name(name),
...
int Persona30::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona30::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona30::setearRelojDeInfeccion(){
...
void Persona30::setearRelojDeVacunacion(){
...
Model &Persona30::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona30::internalFunction(const InternalMessage &)
{
	...
	Model &Persona30::outputFunction(const CollectMessage &msg)
	{
