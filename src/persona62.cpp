...
Persona62::Persona62(const string &name) :
Atomic(name),
name(name),
...
int Persona62::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona62::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona62::setearRelojDeInfeccion(){
...
void Persona62::setearRelojDeVacunacion(){
...
Model &Persona62::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona62::internalFunction(const InternalMessage &)
{
	...
	Model &Persona62::outputFunction(const CollectMessage &msg)
	{
