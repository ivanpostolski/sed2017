...
Persona5::Persona5(const string &name) :
Atomic(name),
name(name),
...
int Persona5::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona5::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona5::setearRelojDeInfeccion(){
...
void Persona5::setearRelojDeVacunacion(){
...
Model &Persona5::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona5::internalFunction(const InternalMessage &)
{
	...
	Model &Persona5::outputFunction(const CollectMessage &msg)
	{
