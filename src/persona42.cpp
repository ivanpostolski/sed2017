...
Persona42::Persona42(const string &name) :
Atomic(name),
name(name),
...
int Persona42::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona42::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona42::setearRelojDeInfeccion(){
...
void Persona42::setearRelojDeVacunacion(){
...
Model &Persona42::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona42::internalFunction(const InternalMessage &)
{
	...
	Model &Persona42::outputFunction(const CollectMessage &msg)
	{
