...
Persona9::Persona9(const string &name) :
Atomic(name),
name(name),
...
int Persona9::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona9::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona9::setearRelojDeInfeccion(){
...
void Persona9::setearRelojDeVacunacion(){
...
Model &Persona9::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona9::internalFunction(const InternalMessage &)
{
	...
	Model &Persona9::outputFunction(const CollectMessage &msg)
	{
