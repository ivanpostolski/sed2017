...
Persona61::Persona61(const string &name) :
Atomic(name),
name(name),
...
int Persona61::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona61::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona61::setearRelojDeInfeccion(){
...
void Persona61::setearRelojDeVacunacion(){
...
Model &Persona61::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona61::internalFunction(const InternalMessage &)
{
	...
	Model &Persona61::outputFunction(const CollectMessage &msg)
	{
