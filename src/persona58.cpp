...
Persona58::Persona58(const string &name) :
Atomic(name),
name(name),
...
int Persona58::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona58::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona58::setearRelojDeInfeccion(){
...
void Persona58::setearRelojDeVacunacion(){
...
Model &Persona58::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona58::internalFunction(const InternalMessage &)
{
	...
	Model &Persona58::outputFunction(const CollectMessage &msg)
	{
