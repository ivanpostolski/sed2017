...
Persona70::Persona70(const string &name) :
Atomic(name),
name(name),
...
int Persona70::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona70::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona70::setearRelojDeInfeccion(){
...
void Persona70::setearRelojDeVacunacion(){
...
Model &Persona70::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona70::internalFunction(const InternalMessage &)
{
	...
	Model &Persona70::outputFunction(const CollectMessage &msg)
	{
