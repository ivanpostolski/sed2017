...
Persona51::Persona51(const string &name) :
Atomic(name),
name(name),
...
int Persona51::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona51::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona51::setearRelojDeInfeccion(){
...
void Persona51::setearRelojDeVacunacion(){
...
Model &Persona51::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona51::internalFunction(const InternalMessage &)
{
	...
	Model &Persona51::outputFunction(const CollectMessage &msg)
	{
