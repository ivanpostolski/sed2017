...
Persona20::Persona20(const string &name) :
Atomic(name),
name(name),
...
int Persona20::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona20::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona20::setearRelojDeInfeccion(){
...
void Persona20::setearRelojDeVacunacion(){
...
Model &Persona20::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona20::internalFunction(const InternalMessage &)
{
	...
	Model &Persona20::outputFunction(const CollectMessage &msg)
	{
