...
Persona13::Persona13(const string &name) :
Atomic(name),
name(name),
...
int Persona13::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona13::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona13::setearRelojDeInfeccion(){
...
void Persona13::setearRelojDeVacunacion(){
...
Model &Persona13::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona13::internalFunction(const InternalMessage &)
{
	...
	Model &Persona13::outputFunction(const CollectMessage &msg)
	{
