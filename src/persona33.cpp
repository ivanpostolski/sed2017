...
Persona33::Persona33(const string &name) :
Atomic(name),
name(name),
...
int Persona33::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona33::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona33::setearRelojDeInfeccion(){
...
void Persona33::setearRelojDeVacunacion(){
...
Model &Persona33::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona33::internalFunction(const InternalMessage &)
{
	...
	Model &Persona33::outputFunction(const CollectMessage &msg)
	{
