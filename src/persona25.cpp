...
Persona25::Persona25(const string &name) :
Atomic(name),
name(name),
...
int Persona25::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona25::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona25::setearRelojDeInfeccion(){
...
void Persona25::setearRelojDeVacunacion(){
...
Model &Persona25::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona25::internalFunction(const InternalMessage &)
{
	...
	Model &Persona25::outputFunction(const CollectMessage &msg)
	{
