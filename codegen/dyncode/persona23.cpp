...
Persona23::Persona23(const string &name) :
Atomic(name),
name(name),
...
int Persona23::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona23::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona23::setearRelojDeInfeccion(){
...
void Persona23::setearRelojDeVacunacion(){
...
Model &Persona23::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona23::internalFunction(const InternalMessage &)
{
	...
	Model &Persona23::outputFunction(const CollectMessage &msg)
	{
