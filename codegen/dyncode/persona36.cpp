...
Persona36::Persona36(const string &name) :
Atomic(name),
name(name),
...
int Persona36::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona36::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona36::setearRelojDeInfeccion(){
...
void Persona36::setearRelojDeVacunacion(){
...
Model &Persona36::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona36::internalFunction(const InternalMessage &)
{
	...
	Model &Persona36::outputFunction(const CollectMessage &msg)
	{
