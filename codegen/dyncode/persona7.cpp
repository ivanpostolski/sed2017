...
Persona7::Persona7(const string &name) :
Atomic(name),
name(name),
...
int Persona7::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona7::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona7::setearRelojDeInfeccion(){
...
void Persona7::setearRelojDeVacunacion(){
...
Model &Persona7::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona7::internalFunction(const InternalMessage &)
{
	...
	Model &Persona7::outputFunction(const CollectMessage &msg)
	{
