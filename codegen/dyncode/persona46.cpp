...
Persona46::Persona46(const string &name) :
Atomic(name),
name(name),
...
int Persona46::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona46::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona46::setearRelojDeInfeccion(){
...
void Persona46::setearRelojDeVacunacion(){
...
Model &Persona46::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona46::internalFunction(const InternalMessage &)
{
	...
	Model &Persona46::outputFunction(const CollectMessage &msg)
	{
