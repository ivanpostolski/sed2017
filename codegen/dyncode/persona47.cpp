...
Persona47::Persona47(const string &name) :
Atomic(name),
name(name),
...
int Persona47::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona47::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona47::setearRelojDeInfeccion(){
...
void Persona47::setearRelojDeVacunacion(){
...
Model &Persona47::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona47::internalFunction(const InternalMessage &)
{
	...
	Model &Persona47::outputFunction(const CollectMessage &msg)
	{
