...
Persona15::Persona15(const string &name) :
Atomic(name),
name(name),
...
int Persona15::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona15::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona15::setearRelojDeInfeccion(){
...
void Persona15::setearRelojDeVacunacion(){
...
Model &Persona15::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona15::internalFunction(const InternalMessage &)
{
	...
	Model &Persona15::outputFunction(const CollectMessage &msg)
	{
