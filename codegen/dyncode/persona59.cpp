...
Persona59::Persona59(const string &name) :
Atomic(name),
name(name),
...
int Persona59::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona59::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona59::setearRelojDeInfeccion(){
...
void Persona59::setearRelojDeVacunacion(){
...
Model &Persona59::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona59::internalFunction(const InternalMessage &)
{
	...
	Model &Persona59::outputFunction(const CollectMessage &msg)
	{
