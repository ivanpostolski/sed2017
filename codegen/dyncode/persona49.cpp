...
Persona49::Persona49(const string &name) :
Atomic(name),
name(name),
...
int Persona49::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona49::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona49::setearRelojDeInfeccion(){
...
void Persona49::setearRelojDeVacunacion(){
...
Model &Persona49::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona49::internalFunction(const InternalMessage &)
{
	...
	Model &Persona49::outputFunction(const CollectMessage &msg)
	{
