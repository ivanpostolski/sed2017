...
Persona60::Persona60(const string &name) :
Atomic(name),
name(name),
...
int Persona60::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona60::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona60::setearRelojDeInfeccion(){
...
void Persona60::setearRelojDeVacunacion(){
...
Model &Persona60::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona60::internalFunction(const InternalMessage &)
{
	...
	Model &Persona60::outputFunction(const CollectMessage &msg)
	{
