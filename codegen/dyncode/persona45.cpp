...
Persona45::Persona45(const string &name) :
Atomic(name),
name(name),
...
int Persona45::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona45::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona45::setearRelojDeInfeccion(){
...
void Persona45::setearRelojDeVacunacion(){
...
Model &Persona45::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona45::internalFunction(const InternalMessage &)
{
	...
	Model &Persona45::outputFunction(const CollectMessage &msg)
	{
