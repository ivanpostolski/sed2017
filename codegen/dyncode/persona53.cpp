...
Persona53::Persona53(const string &name) :
Atomic(name),
name(name),
...
int Persona53::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona53::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona53::setearRelojDeInfeccion(){
...
void Persona53::setearRelojDeVacunacion(){
...
Model &Persona53::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona53::internalFunction(const InternalMessage &)
{
	...
	Model &Persona53::outputFunction(const CollectMessage &msg)
	{
