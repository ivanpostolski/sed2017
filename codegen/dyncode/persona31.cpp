...
Persona31::Persona31(const string &name) :
Atomic(name),
name(name),
...
int Persona31::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona31::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona31::setearRelojDeInfeccion(){
...
void Persona31::setearRelojDeVacunacion(){
...
Model &Persona31::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona31::internalFunction(const InternalMessage &)
{
	...
	Model &Persona31::outputFunction(const CollectMessage &msg)
	{
