...
Persona48::Persona48(const string &name) :
Atomic(name),
name(name),
...
int Persona48::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona48::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona48::setearRelojDeInfeccion(){
...
void Persona48::setearRelojDeVacunacion(){
...
Model &Persona48::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona48::internalFunction(const InternalMessage &)
{
	...
	Model &Persona48::outputFunction(const CollectMessage &msg)
	{
