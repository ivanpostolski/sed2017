...
Persona14::Persona14(const string &name) :
Atomic(name),
name(name),
...
int Persona14::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona14::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona14::setearRelojDeInfeccion(){
...
void Persona14::setearRelojDeVacunacion(){
...
Model &Persona14::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona14::internalFunction(const InternalMessage &)
{
	...
	Model &Persona14::outputFunction(const CollectMessage &msg)
	{
