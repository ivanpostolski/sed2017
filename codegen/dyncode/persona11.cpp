...
Persona11::Persona11(const string &name) :
Atomic(name),
name(name),
...
int Persona11::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona11::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona11::setearRelojDeInfeccion(){
...
void Persona11::setearRelojDeVacunacion(){
...
Model &Persona11::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona11::internalFunction(const InternalMessage &)
{
	...
	Model &Persona11::outputFunction(const CollectMessage &msg)
	{
