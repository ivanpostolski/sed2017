...
Persona50::Persona50(const string &name) :
Atomic(name),
name(name),
...
int Persona50::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona50::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona50::setearRelojDeInfeccion(){
...
void Persona50::setearRelojDeVacunacion(){
...
Model &Persona50::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona50::internalFunction(const InternalMessage &)
{
	...
	Model &Persona50::outputFunction(const CollectMessage &msg)
	{
