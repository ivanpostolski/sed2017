...
Persona18::Persona18(const string &name) :
Atomic(name),
name(name),
...
int Persona18::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona18::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona18::setearRelojDeInfeccion(){
...
void Persona18::setearRelojDeVacunacion(){
...
Model &Persona18::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona18::internalFunction(const InternalMessage &)
{
	...
	Model &Persona18::outputFunction(const CollectMessage &msg)
	{
