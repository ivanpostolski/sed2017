...
Persona24::Persona24(const string &name) :
Atomic(name),
name(name),
...
int Persona24::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona24::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona24::setearRelojDeInfeccion(){
...
void Persona24::setearRelojDeVacunacion(){
...
Model &Persona24::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona24::internalFunction(const InternalMessage &)
{
	...
	Model &Persona24::outputFunction(const CollectMessage &msg)
	{
