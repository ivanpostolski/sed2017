...
Persona43::Persona43(const string &name) :
Atomic(name),
name(name),
...
int Persona43::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona43::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona43::setearRelojDeInfeccion(){
...
void Persona43::setearRelojDeVacunacion(){
...
Model &Persona43::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona43::internalFunction(const InternalMessage &)
{
	...
	Model &Persona43::outputFunction(const CollectMessage &msg)
	{
