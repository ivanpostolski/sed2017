...
Persona22::Persona22(const string &name) :
Atomic(name),
name(name),
...
int Persona22::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona22::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona22::setearRelojDeInfeccion(){
...
void Persona22::setearRelojDeVacunacion(){
...
Model &Persona22::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona22::internalFunction(const InternalMessage &)
{
	...
	Model &Persona22::outputFunction(const CollectMessage &msg)
	{
