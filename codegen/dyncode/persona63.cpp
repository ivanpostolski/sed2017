...
Persona63::Persona63(const string &name) :
Atomic(name),
name(name),
...
int Persona63::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona63::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona63::setearRelojDeInfeccion(){
...
void Persona63::setearRelojDeVacunacion(){
...
Model &Persona63::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona63::internalFunction(const InternalMessage &)
{
	...
	Model &Persona63::outputFunction(const CollectMessage &msg)
	{
