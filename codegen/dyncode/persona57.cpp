...
Persona57::Persona57(const string &name) :
Atomic(name),
name(name),
...
int Persona57::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona57::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona57::setearRelojDeInfeccion(){
...
void Persona57::setearRelojDeVacunacion(){
...
Model &Persona57::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona57::internalFunction(const InternalMessage &)
{
	...
	Model &Persona57::outputFunction(const CollectMessage &msg)
	{
