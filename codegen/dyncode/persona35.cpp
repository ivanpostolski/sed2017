...
Persona35::Persona35(const string &name) :
Atomic(name),
name(name),
...
int Persona35::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona35::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona35::setearRelojDeInfeccion(){
...
void Persona35::setearRelojDeVacunacion(){
...
Model &Persona35::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona35::internalFunction(const InternalMessage &)
{
	...
	Model &Persona35::outputFunction(const CollectMessage &msg)
	{
