...
Persona55::Persona55(const string &name) :
Atomic(name),
name(name),
...
int Persona55::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona55::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona55::setearRelojDeInfeccion(){
...
void Persona55::setearRelojDeVacunacion(){
...
Model &Persona55::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona55::internalFunction(const InternalMessage &)
{
	...
	Model &Persona55::outputFunction(const CollectMessage &msg)
	{
