...
Persona66::Persona66(const string &name) :
Atomic(name),
name(name),
...
int Persona66::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona66::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona66::setearRelojDeInfeccion(){
...
void Persona66::setearRelojDeVacunacion(){
...
Model &Persona66::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona66::internalFunction(const InternalMessage &)
{
	...
	Model &Persona66::outputFunction(const CollectMessage &msg)
	{
