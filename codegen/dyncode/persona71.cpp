...
Persona71::Persona71(const string &name) :
Atomic(name),
name(name),
...
int Persona71::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona71::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona71::setearRelojDeInfeccion(){
...
void Persona71::setearRelojDeVacunacion(){
...
Model &Persona71::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona71::internalFunction(const InternalMessage &)
{
	...
	Model &Persona71::outputFunction(const CollectMessage &msg)
	{
