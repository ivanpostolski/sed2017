...
Persona29::Persona29(const string &name) :
Atomic(name),
name(name),
...
int Persona29::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona29::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona29::setearRelojDeInfeccion(){
...
void Persona29::setearRelojDeVacunacion(){
...
Model &Persona29::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona29::internalFunction(const InternalMessage &)
{
	...
	Model &Persona29::outputFunction(const CollectMessage &msg)
	{
