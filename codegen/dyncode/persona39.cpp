...
Persona39::Persona39(const string &name) :
Atomic(name),
name(name),
...
int Persona39::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona39::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona39::setearRelojDeInfeccion(){
...
void Persona39::setearRelojDeVacunacion(){
...
Model &Persona39::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona39::internalFunction(const InternalMessage &)
{
	...
	Model &Persona39::outputFunction(const CollectMessage &msg)
	{
