...
Persona17::Persona17(const string &name) :
Atomic(name),
name(name),
...
int Persona17::infectados(Estado vecinos[]){
	int result = 0;
...
int Persona17::susceptibles(Estado vecinos[]){
	int result = 0;
	for(int i = 0; i < nvecinos;i++){
		if (vecinos[i]==S){
			result++;
...
void Persona17::setearRelojDeInfeccion(){
...
void Persona17::setearRelojDeVacunacion(){
...
Model &Persona17::externalFunction(const ExternalMessage &msg)
{
...
Model &Persona17::internalFunction(const InternalMessage &)
{
	...
	Model &Persona17::outputFunction(const CollectMessage &msg)
	{
